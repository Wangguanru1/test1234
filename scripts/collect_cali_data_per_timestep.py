...
"""
Sample new images from a pre-trained DiT for a specific timestep.
"""
import torch
...
def sample_cali_data_per_batch(input_list_per_batch, batch_size=32, timestep=None):
    """
    x: (N, C, H, W), t: (N,), y: (N,).
    """
    timesteps = len(input_list_per_batch)
    cali_data = []
    print(f"Total timesteps in batch: {timesteps}")

    if timestep is None:
        # Original behavior: sample from all timesteps
        for t in range(timesteps):
            samples_t = input_list_per_batch[t]
            for idx in range(batch_size):
                cali_data.append([samples_t[0][idx], samples_t[1][idx], samples_t[2][idx]])
    else:
        # Sample only from the specified timestep
        if timestep < timesteps:
            samples_t = input_list_per_batch[timestep]
            for idx in range(batch_size):
                cali_data.append([samples_t[0][idx], samples_t[1][idx], samples_t[2][idx]])
        else:
            print(f"Warning: Timestep {timestep} is out of range. Max timestep is {timesteps - 1}. Skipping batch.")
...
def main(args):
    ...
    for batch_idx in pbar:
        ...
        cali_data_per_batch = sample_cali_data_per_batch(input_list, batch_size=args.batch_size, timestep=args.timestep)
        if cali_data_per_batch:
            cali_data.append(cali_data_per_batch)
        model.reset_input_list()
    
    if not cali_data:
        print("No calibration data was collected. Exiting.")
        return
    ...
    if args.timestep is not None:
        filename = f"cali_data_{args.image_size}_{args.num_sampling_steps}_t{args.timestep}.pth"
    else:
        filename = f"cali_data_{args.image_size}_{args.num_sampling_steps}.pth"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--timestep", type=int, default=None, help="Specific timestep to collect calibration data for.")
